import argparse
import json
import sys


class WaggleArgumentParser(argparse.ArgumentParser):
    def error(self, message):
        print(json.dumps({"status": "error", "message": message}))
        sys.exit(2)


def main():
    parser = WaggleArgumentParser(
        description="Waggle - stateless stdio MCP server for Claude Code worker agents",
        prog="waggle"
    )
    subparsers = parser.add_subparsers(dest="subcommand")

    # waggle sting
    subparsers.add_parser(
        "sting",
        help="Emit waggle CLI reference if waggle MCP is not configured",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        description="Exit silently if waggle MCP is detected, otherwise print CLI reference"
    )

    # waggle mcp
    subparsers.add_parser(
        "mcp",
        help="Launch the Waggle stdio MCP server",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        description="Start the Waggle stdio MCP server. Reads from stdin, writes to stdout.",
    )

    # waggle install
    install_parser = subparsers.add_parser(
        "install",
        help="Install Claude Status hooks and wire Waggle's env vars",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        description=(
            "Verify claude-status is on PATH and run 'claude-status install-hooks' "
            "with CLAUDE_STATUS_RELAY_MODE=on and CLAUDE_STATUS_AUQ_MODE=record."
        ),
    )
    install_parser.add_argument(
        "--auq-order",
        dest="auq_order",
        default=None,
        help="Forwarded to 'claude-status install-hooks --auq-order'. "
             "E.g. 'before:waggle', 'after:waggle', or 'last'.",
    )

    args = parser.parse_args()

    if args.subcommand is None:
        parser.print_help()
        sys.exit(0)
    elif args.subcommand == "sting":
        from waggle.sting import handle_sting
        handle_sting(args)
    elif args.subcommand == "mcp":
        from waggle.mcp_stdio import run
        run()
    elif args.subcommand == "install":
        from waggle.installer import handle_install
        handle_install(args)
